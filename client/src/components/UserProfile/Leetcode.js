...
const LcStrips = ({ user }) => {
  return (
    <>
      ...
      <LcProfileStripData type="website" desc={user.profile.websites[0]} />
      <LcProfileStripData type="github" desc={user.githubUrl} />
      <LcProfileStripData type="twitter" desc={user.linkedinUrl} />
      <LcProfileStripData type="linkedin" desc={user.twitterUrl} />
        ...
        return (
          <div className="lc-profile__language">
            ...
            <div className="lc-profile__language--solved">
              {language.problemsSolved} <span>problem{language.problemsSolved>1 && 's'} solved</span>
            </div>
          </div>
        );
      })}
        ...
        taggedProblemsCounts.map((tag) => {
          return (
            <div className="lc-profile__tag">
              <div className="lc-profile__tag--name">
                {tag.tagName}
              </div>
        ...
        Object.entries(skills).map((entry)=> {
          return (
            <div className="lc-profile__skill"> 
                  <div className={`lc-profile__skill--title lc-profile__skill--${entry[0]}`}>
                    {entry[0]}
                  </div>
  ...
  return (
    <div className="lc-profile__left lc-section">
      <div className="lc-profile__header">
        <img className="lc-profile__header--image" src={user.profile.userAvatar} alt="" />
        <div className="lc-profile__identity">
          <div className="lc-profile__identity--details">
            <h4 className="lc-profile__identity--name">{user.profile.realName}</h4>
            <p className="lc-profile__identity--username">{user.username}</p>
          </div>
          <div className="lc-profile__identity--ranking"><span>Rank</span> {user.profile.ranking}</div>
        </div>
      </div>
...
const RightProfileUi = ({contests, contestHistogram, calendarData, recentSubmissions}) => {
  return <div className="lc-profile__right">
      ...
      {isLoading ? (
        ...
        <>
          ...
          <RightProfileUi 
            ...
            contestHistogram={contestRatingHistogram && contestRatingHistogram.contestRatingHistogram} 
            calendarData = {userProfileCalendar && userProfileCalendar.matchedUser && userProfileCalendar.matchedUser.userCalendar}
